                ...
                <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md animate-in fade-in duration-200">
                    <div className="w-full max-w-md bg-white rounded-[40px] shadow-2xl overflow-hidden animate-in slide-in-from-bottom-5 duration-300">
                        ...
                        <div className="p-8">
                            {contactTab === "menu" && (
                                <div className="space-y-4">
                                    <a href={`https://wa.me/18493876044?text=Hola,%20soy%20${encodeURIComponent(userName)}%20de%20Kinder%20Hive%20Hub,%20deseo%20información.`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-4 p-5 bg-[#25D366]/10 hover:bg-[#25D366]/20 rounded-[24px] transition-all group cursor-pointer">
                            ...
                            {contactTab === "cita" && !citaOk && (
                                <form onSubmit={async (e) => { 
                                    e.preventDefault(); 
                                    const motivo = (e.currentTarget.elements.namedItem('motivo') as HTMLTextAreaElement).value;
                                    setIsReporting(true);
                                    const res = await solicitarReunionAction(motivo);
                                    setIsReporting(false);
                                    if (res.error) {
                                        alert(res.error);
                                    } else {
                                    ...
                                    <div>
                                        ...
                                        <textarea name="motivo" rows={3} required className="w-full mt-1 p-4 rounded-3xl border-2 border-slate-100 font-medium text-slate-700 focus:border-[#8A2BE2] outline-none" placeholder="Explique brevemente el motivo..."></textarea>
                                    </div>
                                    <button type="submit" disabled={isReporting} className="w-full h-14 bg-[#8A2BE2] hover:bg-[#7726c5] text-white font-black rounded-2xl shadow-lg shadow-[#8A2BE2]/20">
                                        {isReporting ? "Enviando..." : "Confirmar Solicitud"}
                                    </button>
                                </form>
                            )}
                            {contactTab === "cita" && citaOk && (
