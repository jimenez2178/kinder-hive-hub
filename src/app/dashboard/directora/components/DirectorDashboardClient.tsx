                ...
                >
                    ...
                    >
                        ...
                        >
                            ...
                            {/* Modal de Confirmación de Cita (Sub-modal) */}
                            {confirmingMeeting && (
                                <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
                                    <div className="bg-white w-full max-w-md rounded-[40px] shadow-2xl overflow-hidden p-8 animate-in zoom-in-95 duration-200">
                                        <h3 className="text-2xl font-black text-[#002147] mb-2 italic">Confirmar Cita</h3>
                                        <p className="text-slate-500 font-bold text-sm mb-6">Para {confirmingMeeting.padre?.nombre_completo}</p>
                                        ...
                                        <form onSubmit={async (e) => {
                                            ...
                                            <div className="grid grid-cols-2 gap-4">
                                                <div>
                                                    ...
                                                    <Input type="date" name="fecha" required className="h-12 rounded-2xl font-bold border-2 focus:border-[#002147]" />
                                                </div>
                                                <div>
                                                    <Label className="text-[10px] font-black uppercase text-slate-400 ml-1">Hora</Label>
                                                    <Input type="time" name="hora" required className="h-12 rounded-2xl font-bold border-2 focus:border-[#002147]" />
                                                </div>
                                            </div>
                                            <div>
                                                <Label className="text-[10px] font-black uppercase text-slate-400 ml-1">Lugar / Comentario</Label>
                                                <textarea name="comentario" required rows={3} className="w-full mt-1 p-4 rounded-3xl border-2 border-slate-100 font-medium text-slate-700 focus:border-[#002147] outline-none" placeholder="Ej: En la dirección, Aula de música, etc."></textarea>
                                            </div>
                                            <div className="flex gap-3 pt-4">
                                                <Button type="button" onClick={() => setConfirmingMeeting(null)} variant="ghost" className="flex-1 h-14 rounded-2xl font-black text-slate-400 hover:text-slate-600">
                                                    Cancelar
                                                </Button>
                                                <Button type="submit" disabled={isLoading} className="flex-1 h-14 rounded-2xl bg-[#002147] hover:bg-black text-white font-black shadow-lg">
