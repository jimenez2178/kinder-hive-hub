...
-- on pagos, galeria, estudiantes, familia_estudiante
-- =============================================
...
CREATE POLICY "Directora can manage family links"
  ON public.familia_estudiante FOR ALL TO authenticated
  USING (has_role(auth.uid(), 'directora'::app_role));

CREATE POLICY "Users can view own family links"
  ON public.familia_estudiante FOR SELECT TO authenticated
  USING (auth.uid() = user_id);

-- Also fix other tables that have the same issue
-- ALERTAS
...
CREATE POLICY "Directora can manage alerts"
...
CREATE POLICY "Maestro can insert alerts"
...
CREATE POLICY "Directora can manage comunicados"
  ON public.comunicados FOR ALL TO authenticated
  USING (has_role(auth.uid(), 'directora'::app_role))
  WITH CHECK (has_role(auth.uid(), 'directora'::app_role));

CREATE POLICY "Everyone can view comunicados"
  ON public.comunicados FOR SELECT TO authenticated
  USING (true);

CREATE POLICY "Staff can create comunicados"
...
CREATE POLICY "Directora can manage birthdays"
  ON public.cumpleanos FOR ALL TO authenticated
  USING (has_role(auth.uid(), 'directora'::app_role))
  WITH CHECK (has_role(auth.uid(), 'directora'::app_role));

CREATE POLICY "Everyone can view birthdays"
  ON public.cumpleanos FOR SELECT TO authenticated
  USING (true);
...
CREATE POLICY "Directora can manage messages"
  ON public.mensaje_dia FOR ALL TO authenticated
  USING (has_role(auth.uid(), 'directora'::app_role))
  WITH CHECK (has_role(auth.uid(), 'directora'::app_role));

CREATE POLICY "Everyone can view messages"
  ON public.mensaje_dia FOR SELECT TO authenticated
  USING (true);
...
CREATE POLICY "Staff can manage notes"
  ON public.notas_maestras FOR ALL TO authenticated
  USING (has_role(auth.uid(), 'directora'::app_role) OR has_role(auth.uid(), 'maestro'::app_role))
  WITH CHECK (has_role(auth.uid(), 'directora'::app_role) OR has_role(auth.uid(), 'maestro'::app_role));

CREATE POLICY "Everyone can view notes"
  ON public.notas_maestras FOR SELECT TO authenticated
  USING (true);
...
CREATE POLICY "Anyone can create thanks"
  ON public.agradecimientos FOR INSERT TO authenticated
  WITH CHECK (true);

CREATE POLICY "Directora can delete thanks"
  ON public.agradecimientos FOR DELETE TO authenticated
  USING (has_role(auth.uid(), 'directora'::app_role));

CREATE POLICY "Everyone can view thanks"
  ON public.agradecimientos FOR SELECT TO authenticated
  USING (true);
