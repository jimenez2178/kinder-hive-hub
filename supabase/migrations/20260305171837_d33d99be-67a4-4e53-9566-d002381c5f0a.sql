...
CREATE POLICY "Everyone can view comunicados" ON public.comunicados FOR SELECT TO authenticated USING (true);
...
CREATE POLICY "Directora can update comunicados" ON public.comunicados FOR UPDATE TO authenticated
  USING (has_role(auth.uid(), 'directora'::app_role)) WITH CHECK (has_role(auth.uid(), 'directora'::app_role));
...
CREATE POLICY "Directora can manage birthdays" ON public.cumpleanos FOR ALL TO authenticated
  USING (has_role(auth.uid(), 'directora'::app_role)) WITH CHECK (has_role(auth.uid(), 'directora'::app_role));
CREATE POLICY "Everyone can view birthdays" ON public.cumpleanos FOR SELECT TO authenticated USING (true);
...
CREATE POLICY "Directora can manage students" ON public.estudiantes FOR ALL TO authenticated
  USING (has_role(auth.uid(), 'directora'::app_role)) WITH CHECK (has_role(auth.uid(), 'directora'::app_role));
CREATE POLICY "Everyone can view students" ON public.estudiantes FOR SELECT TO authenticated USING (true);
...
CREATE POLICY "Everyone can view events" ON public.eventos FOR SELECT TO authenticated USING (true);
...
CREATE POLICY "Everyone can view gallery" ON public.galeria FOR SELECT TO authenticated USING (true);
...
CREATE POLICY "Everyone can view notes" ON public.notas_maestras FOR SELECT TO authenticated USING (true);
...
CREATE POLICY "Everyone can view thanks" ON public.agradecimientos FOR SELECT TO authenticated USING (true);
CREATE POLICY "Anyone can create thanks" ON public.agradecimientos FOR INSERT TO authenticated WITH CHECK (true);
...
CREATE POLICY "Everyone can view messages" ON public.mensaje_dia FOR SELECT TO authenticated USING (true);
CREATE POLICY "Directora can manage messages" ON public.mensaje_dia FOR INSERT TO authenticated
  WITH CHECK (has_role(auth.uid(), 'directora'::app_role));
...
-- ===== FAMILIA_ESTUDIANTE =====
DROP POLICY IF EXISTS "Directora can insert family links" ON public.familia_estudiante;
